...
function Addproduct() {
  ...
  const handlesubmit = async (e) => {
    try {
      ...
      const { Pname, PPrice, Desc, Anime, img ,featured} = data
      const response = await fetch("http://localhost:8000/api/products/add", {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
        body: JSON.stringify({ Pname, PPrice, Desc, Anime, img ,featured})
      })
  ...
  return (
    <div>
      <form className='w-full h-auto flex  flex-col justify-center items-center p-14 bg-slate-100' onSubmit={handlesubmit} >
        ...
        <input type="number" name='PPrice' placeholder='Enter Price of Product' onChange={(e) => setdata({ ...data, PPrice: e.target.value })} className='p-2 m-2 border border-black' />
